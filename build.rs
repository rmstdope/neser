...
fn remove_trailing_metadata_groups(input: &str) -> String {
    ...
    loop {
        ...
        // Try to remove trailing bracketed/parenthesized groups
        if let Some(new_result) = remove_trailing_group(trimmed, '(', ')') {
            result = new_result;
        } else if let Some(new_result) = remove_trailing_group(trimmed, '[', ']') {
            result = new_result;
        } else {
            break;
        }
...
fn remove_trailing_group(text: &str, open: char, close: char) -> Option<String> {
    ...
    // Check that the brackets/parens are properly nested
    if text[start..].find(close).is_none() {
        return None;
    }
    
    let inner = &text[start + 1..text.len() - 1];
    if inner.chars().any(|c| c == open) {
        return None;
    }
