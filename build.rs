...
fn main() {
    ...
    let code = generate_test_code(&autorun_files);
    fs::write(&generated_path, code).expect("write generated autorun tests");
}

fn generate_test_code(autorun_files: &[PathBuf]) -> String {
...
fn remove_trailing_metadata_groups(input: &str) -> String {
    ...
    loop {
        ...
        if remove_trailing_group(trimmed, '(', ')') {
...
fn remove_trailing_group(text: &str, open: char, close: char) -> bool {
    ...
    text[start..].find(close).is_some()
        && text[start + 1..text.len() - 1].chars().all(|c| c != open)
}
