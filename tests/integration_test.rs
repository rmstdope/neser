...
// C66E  60        RTS                             A:00 X:FF Y:15 P:27 SP:FD PPU:233,209 CYC:26554";
    ...
    let cartridge = neser::cartridge::Cartridge::new(&rom_data).expect("Failed to parse ROM");

    // Create NES and insert cartridge
    let mut nes = neser::nes::Nes::new();
    ...
    for line in golden_log.lines() {
        ...
        let actual = nes.trace();
        
        // For now, skip PPU and CYC parts
        assert_eq!(expected[..73], actual);
        nes.cpu.run_opcode();
    }
