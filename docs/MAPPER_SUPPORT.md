...
| 343 | Reset-based 4-in-1 multicart (NES 2.0 reassignment) | NROM-128 style per game: selected 16 KB bank mirrored at `$8000` and `$C000` | Selected 8 KB CHR bank per game | Fixed from header | None | None | None | ✅ | Routed as a mapper 60-compatible alias. Internal 2-bit game index advances on soft reset and wraps after 4 entries. |
