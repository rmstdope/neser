...
| 90 | J.Y. Company ASIC (Famicom Zhan Shi, Aladdin, Pocahontas) | 8 KB × 4 (mode 0), 16 KB × 2 (mode 1), 16 KB × 1 + fixed (mode 2), 32 KB (mode 3); modes via `$D000` bits 0–1 | 1 KB × 8 (mode 3), 2 KB × 4 (mode 2), 4 KB × 2 (mode 1), 8 KB × 1 (mode 0); modes via `$D000` bits 2–3; 9-bit banks via LSB (`$9000-$9007`) + MSB (`$A000-$A007`) pairs | V / H / Single-lower / Single-upper via `$D001` bits 0–1 | Prescaler-based; `$C002` enable; `$C003` ack; `$C004-$C007` write prescaler/counter/mode; fires on zero-cross with configurable tick source (CPU, A12, PPU-read, CPU-write) | None | None | ✅ | Hardware multiplier at `$5800`/`$5801` (write factors) read result at `$5800` (lo) / `$5801` (hi). Prescaler XOR register (`$C000`) applied when loading `$C004`/`$C005`. Count direction bits 6–7 of `$C001`: 0/3 = no count, 1 = up, 2 = down. Small prescaler mask `$07` when `$C001` bit 2 set, else `$FF`. |
