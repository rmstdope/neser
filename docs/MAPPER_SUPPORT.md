...
| 34 | BNROM / NINA-001 | BNROM: 32 KB switchable; NINA: 32 KB switchable | BNROM: 8 KB CHR-RAM; NINA: 4 KB × 2 switchable | Fixed (solder pads) | None | None | NINA: 8 KB PRG-RAM | ⚠️ | NES 2.0 submapper 1 = NINA-001, submapper 2 = BNROM. **Without valid submapper, heuristic is used (CHR size ≤ 8 KB → BNROM, > 8 KB → NINA).** Bus conflicts on BNROM not emulated – acceptable. NINA registers at $7FFD/$7FFE/$7FFF overlap PRG-RAM. |
...
| 77 | Irem NINA-03 | Napoleon Senki | Fixed mapper with 8 KB CHR-RAM segment registers |
