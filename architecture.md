...
| `src/snes/bus/system_bus.rs` | `SnesSystemBus` implementation: WRAM direct/mirror mapping, LoROM/HiROM/ExHiROM ROM decode, SRAM windows with wrap semantics, strict MDR/open-bus reads, scoped MMIO support (`$2180-$2183`, mul/div `$4202-$4206`, MEMSEL `$420D`, MDMAEN `$420B`, HDMAEN `$420C`), ownership of the `Ppu` (advanced per `tick`, register routing for `$2100-$213F` + `$4200`/`$4210`/`$4211`/`$4212`, `poll_nmi` delegation, framebuffer + PPU save-state passthrough), plus explicit `hdma_init()` / `hdma_do_line()` APIs used by tests and future PPU scanline wiring. Also owns save-state capture/restore helpers for WRAM, SRAM, MMIO latches, and DMA runtime state. |
...
| `src/snes/ppu/mod.rs` | `Ppu` struct + power-on state, scan-position accessors, NMI/frame-complete/framebuffer accessors, and submodule declarations. The PPU is a dot-based pipeline owned by the bus. |
...
| `src/snes/ppu/timing.rs` | Dot/scanline counters (1 dot per 4 master clocks, 341 dots/line, 262 NTSC lines), VBlank entry/exit (line 225), the NMI rising-edge model (`nmi_enable && nmi_flag`), H/V counter latching (SLHV / WRIO falling edge), and per-dot OBJ-pipeline advancement (`update_obj_pipeline`). |
...
| `src/snes/apu/dsp.rs` | Initial S-DSP voice pipeline core: `$00-$7F` DSP register file, BRR block decode baseline (range/filter/history + loop/end flags), fixed 512-entry clean-room gaussian interpolation LUT path, per-voice pitch accumulator stepping from `VxPITCH`, per-voice/master volume mixing helpers, and backward-compatible state normalization from save-state register payloads. |
