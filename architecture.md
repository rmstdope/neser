...
| `src/snes/integration_tests/peterlemon_ppu_bg_tests.rs` | PeterLemon basic PPU BG demo suite (#2878): 2/4/8bpp tile decoding, BG1-BG4 tilemaps, all four tilemap screen sizes, tile flip, backdrop and palettes. 11 of 12 vendored ROMs have screen-CRC goldens pixel-diff-approved against Mesen2 (and the 8 upstream reference screenshots); `8x8BGMap8BPP32x32.sfc` is excluded pending the animation frame-phase divergence (#2990). |
| `src/snes/integration_tests/undisbeliever_ppu_bg_tests.rs` | undisbeliever PPU BG / VMAIN suite (#2878): VRAM increment modes at all bit depths, byte- vs word-increment uploads, 1bpp tile decode, non-DMA VRAM writes. 8 of 18 locally-built ROMs have Mesen2-approved screen-CRC goldens; the 6 `*-with-remapping` ROMs are excluded pending VMAIN address remapping (#2989) and the scroll/textbuffer ROMs pending #2990. |
