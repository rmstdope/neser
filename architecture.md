...
| `scripts/refresh_65816_processor_tests_subset.sh` | Refreshes the pinned SNES 65816 ProcessorTests subset from upstream (`SingleStepTests/ProcessorTests`), truncating each tracked opcode file to the first 32 vectors for deterministic and fast local verification. |
...
| `src/snes/integration_tests/processor_tests_65816.rs` | Loader and harness for Tom Harte ProcessorTests 65816 JSON vectors. Parses upstream schema (`initial` / `final` / `cycles`), runs single-step CPU checks against a RAM-backed `SnesBus`, and compares final CPU+RAM state and vector cycle count. Includes ignored pinned-subset tests for real vector files. |
...
| `roms/snes/automated_tests/processor_tests/65816/` | Pinned subset of Tom Harte ProcessorTests 65816 vectors (`v1/00.{e,n}.json`, `v1/ea.{e,n}.json`, first 32 vectors each) plus source metadata. Used by ignored SNES integration tests in `src/snes/integration_tests/processor_tests_65816.rs`. |
