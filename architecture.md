...
The SNES (Super Nintendo Entertainment System) module now includes active 65816 CPU execution wired to a functional `SnesSystemBus`, cartridge parsing/mapping for LoROM/HiROM/ExHiROM, both general-purpose DMA and explicit HDMA execution hooks, a versioned JSON save-state path for CPU/bus/PPU/ROM identity capture and restore, and a **dot-based PPU** (#2759, #2760): the bus owns the PPU and advances it one dot per 4 master clocks, routes the `$2100-$213F` register file plus `$4200`/`$4210`/`$4211`/`$4212`, latches the H/V counters, raises VBlank NMI to the CPU via `SnesBus::poll_nmi`, and renders the **BG tile pipeline for Modes 0/1** (per-dot fetch+composite: 8x8/16x16 tiles, 2bpp/4bpp, all tilemap sizes with SC mirroring, scroll, flip, per-tile/BG3 priority, TM enable) over the backdrop with master brightness/forced-blank to a frame-paced framebuffer. Remaining BG modes 2-7, OBJ/sprites, windows/color math, mosaic, and IRQ timing, plus APU/input, remain incremental; HDMA scanline scheduling is intentionally exposed as bus APIs (`hdma_init` / `hdma_do_line`) until per-scanline PPU wiring lands.
...
| `src/snes/ppu/registers.rs` | PPU register read/write dispatch (`$2100-$213F`, `$4200`/`$4201`/`$4210`/`$4212`) and VRAM/CGRAM/OAM access: word addressing, VMAIN increment + prefetch-on-read, CGRAM/OAM write latches, OAM high-table addressing, H/V counter latch reads (OPHCT/OPVCT), STAT77/STAT78, RDNMI, HVBJOY. Also dispatches the BG registers (BGMODE, BGnSC, BGxxNBA, BGnHOFS/VOFS, TM). |
| `src/snes/ppu/background.rs` | BG tile pipeline for Modes 0/1: BGnHOFS/VOFS shared write-twice (BG_old) scroll latch, and the per-dot fetch+composite (`compute_pixel`) — 8x8/16x16 tile decode (2bpp/4bpp), 32x32/64x32/32x64/64x64 tilemaps with SC0..SC3 mirroring, H/V flip, per-tile + Mode-1 BG3 high priority, per-BG TM enable, Mode-0 per-BG palette regions, backdrop fallback. |
