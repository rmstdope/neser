...
| `src/snes/apu/mod.rs` | `SnesApu` path with initial S-DSP voice-pipeline wiring: owns 64 KB ARAM, embedded/override IPL ROM selection, SPC700 CPU stepping via master-clock catch-up, `$2140-$2143` <-> `$F4-$F7` port handshake latches, DSP register ports `$F2/$F3`, SPC700 timer integration (`$F1/$FA-$FF`), per-SPC-cycle 32-phase DSP skeleton stepping, and save-state capture/restore including DSP state. |
| `src/snes/apu/dsp/` | S-DSP voice pipeline split into focused submodules (`mod.rs`, `voice.rs`, `envelope.rs`, `brr.rs`, `gaussian.rs`) with a per-voice state model. Implements `$00-$7F` register file behavior for ADSR/GAIN envelope stepping, KON/KOFF latency handling, NON noise routing with FLG-selected clocking, PMON adjacent-voice pitch modulation, ENVX/OUTX updates, BRR decode baseline, gaussian interpolation, pitch accumulation, per-voice/master volume helpers, and backward-compatible restore normalization for additive DSP state fields. |
...
| `src/snes/input/mod.rs` | SNES input subsystem: the `SnesController` trait, `SnesButton`/`button_from_id` (12-button mapping with X=10/Y=11), `SnesControllerType` (standard/multitap/mouse/superscope; non-standard fall back to standard for now), and `InputPorts` — the two controller ports plus the auto-joypad sequencer. Routes manual serial `$4016`/`$4017` (shared shift register, `$4016.0` strobe, grounded `$4017` bits 2-4, open-bus upper bits) and automatic reading into JOY1-JOY4 (`$4218-$421F`, enabled by `$4200.0`, busy via `$4212.0`, latched input committed after the 4224-master-cycle window). Save-state via `InputPortsState`. |
