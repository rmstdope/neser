...
| `src/snes/integration_tests/undisbeliever_tests.rs` | undisbeliever/snes-test-roms hardware-glitch/timing-hammer suite. Unlike blargg/gilyon these ROMs print no PASS/FAIL text, so each of the 11 automated tests (of 29 vendored ROMs) asserts a stability-snapshot screen CRC via the `rom_runner` oracle, cross-checked against Mesen2. The other 18 ROMs showed real divergences from Mesen2 and are deliberately left un-automated pending #2943/#2944/#2945. |
