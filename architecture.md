...
| `src/gba/bus/mod.rs` | `GbaBus` — system memory bus implementing the CPU `Bus` trait with full GBA address-space routing (BIOS, EWRAM, IWRAM, I/O, PRAM, VRAM, OAM, cart ROM/SRAM), open-bus modelling, BIOS lock, VRAM mirroring, N/S wait-state cycle stubs, and save-state capture/restore for bus-owned memory/peripheral state. |
