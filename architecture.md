...
| `src/snes/integration_tests/kungfufurby_hdma_tests.rs` | KungFuFurby HDMA ROMs (#2884): `test_hdma`/`test_hdmasync`/`test_hdmatiming`, all `#[ignore]`d recording NESER's diverging screen CRC (black or red-FAIL) where Mesen2 renders a blue PASS, rooted in HDMA H-position timing (#3062, #3042/#3050). |
| `src/snes/integration_tests/jonasquinn_dma_tests.rs` | Canonical jonasquinn DMA/HDMA ROMs (#2884): `test_mdrhdma` and `hdma_midframe` carry 0-pixel-diff Mesen2 goldens; `test_dmavalid`/`test_hdmadisable`/`test_dmatiming` are `#[ignore]`d divergences (#3063), and the `test_hdmasync`/`test_hdmatiming` byuu mirrors share #3062. |
