...
| `scripts/reference_capture/` | Verified headless screenshot recipes for the screenshot-reference emulators (`README.md`), the Lua capture scripts for Mesen2 (`mesen2_capture.lua`, NES and SNES) and `mgba-headless` (`mgba_capture.lua`), and the one-hunk patch that gives stock `mgba-headless` a framebuffer. Every hardware research skill's Tier 3 points here. |
...
| `src/platform/config/` | `FrontendConfig` struct — generic frontend settings shared across all emulated systems — split into per-domain modules: `mod.rs` (data structs + thin `apply_args`/`apply_config_value` orchestrators), `cli.rs` (shared CLI machinery: flag table, parse helpers, validation, help text), and `audio`/`video`/`autorun`/`debugger`/`cartridge`/`headless` (per-domain flag parsing). The `cartridge` module holds the `resolved_metadata_db_path()`, `resolved_image_cache_path()`, and `resolved_favorites_path()` helpers. The `headless` module parses `--headless`/`--frames`/`--output` into a `HeadlessCapture` and enforces the rules that keep a capture reproducible (zero RAM init, no autorun or TUI combination). |
| `src/platform/headless_capture.rs` | Headless frame-capture runner behind `--headless`. Loads a ROM, advances a fixed number of frames with no window or audio, and writes the final frame as a PNG. `run_if_requested()` is the dispatch entry point called from `main.rs`; the frame loop is bounded by `MAX_TICKS_PER_FRAME` so a ROM that never renders fails instead of hanging an unattended script. |
...
| `src/platform/rom_loader.rs` | Shared "ROM path to ready-to-run `Console`" loading — `detect_system_type()` (by file extension) and `load_console()`. Used by both the native frontend and headless capture so the two cannot drift; resetting is left to the caller. |
...
| `src/nes/ppu/system_palettes.rs` | Preset NES system palettes (`NesPalette` enum + RGB tables), including Mesen2's default (`mesen`) for comparing captures with Mesen2. Selectable via `nes-palette` config and cycled at runtime with F8. |
...
| `src/snes/console/snes.rs` | `Snes` — platform-facing SNES wrapper implementing the `Emulator` trait. Owns `Option<Cpu<SnesSystemBus>>`; `load_rom` parses a `Cartridge`, resolves SNES hardware mode (config override, else header-country auto-detect), constructs the system bus/PPU with that region and the configured `ram_init_mode` power-on RAM pattern, and resets CPU state; `reset(false)` (hard reset) re-applies that pattern while `reset(true)` (soft reset) preserves RAM; `run_tick` executes real CPU steps when a ROM is loaded (returns 0 cycles when not loaded) and sets `ready_to_render` once per PPU frame; `screen_snapshot` returns the PPU framebuffer as RGB888; save-state bytes serialize via the versioned snapshot module, and a restore adopts the region the state carries for frame pacing (`target_frame_duration`) as the PPU and APU already do, whatever `snes-hardware` the loading console was configured with (nr-273). |
...
| `src/snes/bus/mod.rs` | `SnesBus` trait definition plus `StubBus`, `TestBus`, `SnesSystemBus` and `SnesBusOptions` exports. The trait defines the memory access contract (`read`, `write`, `tick`) plus `poll_nmi`/`poll_irq` for delivering hardware interrupts to the CPU, mirroring the pattern used in GB and GBA. `poll_nmi` returns the consumed edge's recognition-arm delay in CPU cycles (0 = no edge, default): 1 for the PPU's own vblank edge, 2 for an edge raised by an NMITIMEN enable-mid-vblank write — Mesen2's `SetNmiFlag(1)`/`SetNmiFlag(2)` distinction, hardware-pinned by byuu's test_nmi v1.1 test 27 (#3081). |
| `src/snes/bus/system_bus.rs` | `SnesSystemBus` implementation, constructed from a `SnesBusOptions` (SPC IPL override path, video region, `ram_init_mode`): WRAM direct/mirror mapping, LoROM/HiROM/ExHiROM ROM decode, SRAM windows with wrap semantics, strict MDR/open-bus reads, scoped MMIO support (`$2180-$2183`, mul/div `$4202-$4206`, MEMSEL `$420D`, MDMAEN `$420B`, HDMAEN `$420C`), ownership of the `Ppu` (advanced per `tick`, register routing for `$2100-$213F` + `$4200`/`$4210`/`$4211`/`$4212`, `poll_nmi` delegation, framebuffer + PPU save-state passthrough), and region-aware PPU construction (`NTSC`/`PAL`) at bus init. A clock-based fallback (`run_overdue_pending_dma`) runs an armed transfer for callers that never drive `gpdma_cycle_hook`, and is inert as soon as anything has driven the hook. Before #3074 it also fired on the normal CPU path, running a transfer on the last tick of the start-delay cycle rather than at the start of the next, which put it in a different CPU cycle from the one Mesen2 uses (`ProcessPendingTransfers` runs only from `ProcessCpuCycle`) -- #3065's interrupt-suppression window had been calibrated against that, which is why #3067's attempt to gate the fallback alone had to be reverted. `tick()` calls `check_hdma_triggers()` every master clock (including through the DRAM-refresh stolen-clock window) to ARM `hdma_init()`/`hdma_do_line()` at `Ppu::hdma_init_due()`/`hdma_transfer_due()` (#2947 -- previously unwired, so HDMA never ran during emulation). Armed HDMA and a `$420B` general-purpose transfer both run from `SnesBus::gpdma_cycle_hook`, at the start of the second CPU cycle after their trigger, with HDMA taking priority (#3021, Mesen2 `ProcessPendingTransfers`). The hook RETURNS whether it ran a transfer -- Mesen2's `IrqLock = ProcessPendingTransfers()` -- and both kinds report it, so an HDMA burst locks interrupt recognition for its cycle exactly as a general-purpose transfer does (#3074); the per-line HDMA transfer is only armed when HDMAEN is non-zero at the trigger clock, so a channel enabled mid-scanline waits for the next line. `DmaABus::dma_write_b_bus` routes WMDATA/WMADD (`$2180-$2183`) through `write_mmio` so DMA writes get the same auto-incrementing WRAM port a direct CPU store would (#2945 -- previously silently dropped). CPU-status reads preserve open-bus bits while still honoring register side effects (including `$2137` strobe latching). Also owns save-state capture/restore helpers for WRAM, SRAM, MMIO latches, and DMA runtime state. |
| `src/snes/bus/dma.rs` | `DmaController` for #2745/#2746: `$43x0-$43xB` channel register file, general-purpose DMA execution, HDMA per-frame init and per-line execution (direct+indirect tables, repeat/continue semantics, terminator handling, channel 0→7 ordering). Both transfer directions go through the real bus: A→B via `DmaABus::dma_write_b_bus`, and B→A via `DmaABus::dma_read_b_bus`, which reads the live `$2100-$21FF` register with its real read side effects (VRAM prefetch reload/increment, OAM/CGRAM auto-increment, OPHCT/OPVCT flip-flops, WMDATA/WMADD auto-increment) and yields the DMA's own open-bus byte for ports with no read driver. Until #3061 the controller instead kept a 256-byte `bbus_ports` mirror of every A→B write and served B→A reads from it, so reading any port that had not been written A→B returned 0; that mirror (and its `SnesDmaState` field) is gone. Both directions share one per-byte slot helper, `copy_dma_byte`, which is also where the WRAM-to-WRAM restriction lives (#3111, fullsnes "DMA Notes"): a slot whose B-bus address is WMDATA (`$80`) and whose A-bus address decodes to WRAM is refused, because the WRAM chip cannot service the A-bus and the `$2180` port at once. A→B performs neither access; B→A skips the `$2180` read (so WMADD does not advance) but still writes the A-bus, depositing `INVALID_WRAM_TRANSFER_BYTE` (`0xFF`, following Mesen2's `CopyDmaByte`; ares writes `0x00` and neither is hardware-verified). Either way the slot keeps its full 8 clocks, the DMA open bus is untouched, and the channel's address/count bookkeeping still runs — byuu's `test_dmavalid.smc` checks all three. The predicate is `DmaABus::dma_a_bus_is_wram` (a pure address decode; `SnesSystemBus` delegates to `decode_wram_index`), and it is tested against the slot's own offset B-bus address, not the channel's raw BBAD, so a mode-1 channel at BBAD `$7F` refuses only every other slot. HDMA table and indirect-descriptor fetches are controller-internal A-bus reads rather than transfer bytes, and are deliberately not guarded. Every transfer runs inside the **hardware start/end envelope** (#3021, ported from Mesen2's `SnesDmaController`): a `SyncStartDma` pad of 1-8 clocks to the next 8-master-clock boundary, 8 clocks of setup, 8 clocks per channel plus 8 per byte, and a `SyncEndDma` pad rounding the charged total to a whole CPU cycle -- for the two HDMA envelopes at the speed of the access the transfer is standing in front of (6/8/12, published by the CPU through `SnesBus::set_cpu_speed` before every cycle hook, mirroring Mesen2's `SetCpuSpeed`-before-`ProcessCpuCycle` ordering), for **all three** envelopes at the speed of the access the transfer is standing in front of. General-purpose DMA rounded to a fixed 8 until #3127, which also established why that literal had survived two prior investigations. The controller now tracks two totals (`DmaClocks`): `ticks`, the real master clocks spent, and `align`, Mesen2's `_dmaClockCounter` -- the value `SyncEndDma` divides. They differ deliberately, because Mesen2 counts a general-purpose channel's bytes in a `uint8_t` and charges `_dmaClockCounter += 8 * i` after the loop, so a >= 256-byte transfer contributes a multiple of 2048 less than its real cost (ares does not do this: `counter.dma` is a `u32`, so the wrap is a Mesen2 artifact, reproduced deliberately because every committed screen golden is a Mesen2 capture). `8 * n` is a multiple of 8 for every `n`, so that mismatch is invisible while the pad rounds to 8 and appears only at 6 or 12 -- the fixed 8 was compensating for the counter, not modelling the pad. Verified by mutation: exact byte counting plus the speed-aware divisor reproduces #3067's recorded outcome exactly (jonasquinn `test_dmatiming/demo.smc` correct, `mosaic_mode5_sized` failing), and with the wrap both are exact. The re-entrancy the previous note named as the live hypothesis was falsified (`demo.smc` never writes `$420C`) and is now implemented regardless -- see below. Rounding HDMA to a fixed 8 made every transfer landing in front of a 6-clock register access two clocks too long, which is what pushed StarWars.sfc's `$4210` poll read past the 4-clock RDNMI hold window and cost one Mode 7 zoom step per frame. **HDMA falling due inside a general-purpose transfer runs nested** (#3127, previously unmodelled): `SnesSystemBus::dma_tick` arms triggers the burst sweeps past, and the controller claims them through `DmaABus::take_due_hdma` at Mesen2's three re-entry points -- after the start overhead, after each channel's overhead, and after every byte. The nested run pays neither sync pad (`need_sync = false`, matching `needSync = !HasActiveDmaChannel()` and ares' `if(!dmaEnable())` guards) and folds its clocks into the outer `align`, which is rounded once at the end. An HDMA-enabled channel also has its running general-purpose transfer aborted mid-flight via `dma_active_mask` (Mesen2's per-channel `DmaActive`), leaving `$43x5` non-zero. Before this, `dma_tick` polled no triggers at all, so a 64 KB transfer silently swallowed ~6 scanlines of HDMA. HDMA's per-line work is two-phase -- all channel transfers first, then per-channel bookkeeping with a speculative descriptor read every line (pointer advanced only on expiry), indirect pointer loads before the termination check, and the last-active-channel single-MSB oddity. Clocks are spent live through `DmaABus::dma_tick` (so general-purpose DMA pays the once-per-scanline 40-clock DRAM-refresh stall mid-transfer, #2985), and each B-bus write lands directly at its true bus clock -- 4 clocks to the A-bus read, 4 more before the write -- which replaced #3020's per-byte deadline queue. The last visible pixel (x = 255) still renders pre-write because the burst only starts two CPU cycles after the dot-276 trigger. |
...
| `src/snes/cpu/cpu.rs` | `Cpu<B: SnesBus>` -- the 65816 core (also reused by SA-1's second CPU, `src/snes/sa1/mod.rs`). One `step()` call executes one full instruction (fetch/decode/execute), except WAI idling and in-progress MVN/MVP block moves which each spend exactly one `step()` call per cycle/byte, and STP halting. STP (`stopped`, #3116) models Mesen2's `SnesCpuStopState::Stopped`: each `step()` advances the bus by 4 master clocks and returns without beginning a CPU cycle, mirroring `ProcessHaltedState`'s use of `IncMasterClock4()` rather than `Idle()`/`ProcessCpuCycle()` -- so the PPU keeps displaying while an armed HDMA transfer never runs and no interrupt is polled. Unlike WAI, no interrupt releases it; only `do_reset` does, which also clears `waiting`. Interrupt (NMI/IRQ/ABORT) recognition is checked once per instruction boundary, at the top of `step()`, matching real hardware and Mesen2's `CheckForInterrupts` -- but the *pending state itself* is kept precise at per-CPU-cycle granularity (#3049), hooked into the start/end of the three CPU-cycle-boundary functions (`tick_read`, `tick_write`, `tick_internal_cycle`), mirroring Mesen2's `DetectNmiSignalEdge` (called once per `ProcessCpuCycle` from every `Read`/`Write`/`Idle`), rather than NESER's previous once-per-`step()` poll (which could overshoot dispatch by an entire extra instruction). NMI (edge-triggered) uses `resolve_nmi_arm_counter`/`poll_and_arm_nmi_edge`, an arm-this-cycle/latch-next-cycle counter mirroring Mesen2's `NmiFlagCounter` closely enough to reproduce its exact one-cycle edge-to-latch latency; the arm value comes from `SnesBus::poll_nmi` (1 for the PPU's vblank edge, 2 for an NMITIMEN enable-mid-vblank edge — Mesen2's `SetNmiFlag(2)`, so a wide `$4200` store whose enabling write lands on its second-to-last cycle still lets the following instruction complete, byuu test_nmi v1.1 test 27). #3081 deleted the earlier `irq_lock_step` mechanism (#2918), an instruction-granular ABORT/NMI/IRQ/WAI-wake lock armed by `$4200`/`$420B` writes with no Mesen2 counterpart; the enable-mid-vblank delay it had been masking is the `SetNmiFlag(2)` port above, and the DMA case is fully covered by the per-cycle lock. A DMA running in a cycle locks interrupt recognition for that one cycle (#3074, Mesen2's `_state.IrqLock`): the bus hook returns whether it ran a transfer, a counter resolving while locked re-arms to 1 instead of setting `nmi_pending`, and `resample_irq_line` yields false. `resolve_nmi_arm_counter` therefore runs just after the hook (Mesen2's `ProcessPendingTransfers` -> `DetectNmiSignalEdge` order) but still before any `bus.tick()`. This replaced #3065's two-cycle `dma_suppress_cycles` window, which had no Mesen2 counterpart and needed a hand-tuned NMI/IRQ asymmetry; with the lock that symmetry falls out of the re-arm. IRQ (level-triggered, already a live non-consuming signal via `Ppu::poll_irq_dispatch`) is sampled into TWO shadows per cycle, because Mesen2 keeps two and they are not interchangeable (#3146). `resample_irq_line` runs at cycle START, before any `bus.tick()`, and feeds the dispatch decision -- Mesen2's `PrevIrqSource`, set in `DetectNmiSignalEdge` from `ProcessCpuCycle` ahead of the cycle's clocks -- so an instruction takes an IRQ only if the line was already up when its final cycle began; a line rising mid-cycle waits for the next boundary. `resample_irq_wai_line` runs at cycle END and feeds only WAI's wake -- Mesen2's `_waiOver` -- keeping the fresher sample that stops a wake costing an extra idle iteration (#3049, whose `inidisp_forgot_to_force_blank.sfc` witness still holds: moving this sample too takes that golden from `0xBB047582` to `0x6E8D8520`). `step()` reads the dispatch shadow AFTER the WAI wake cycles rather than snapshotting it at the top, because the wake cycles re-sample it and on the waking iteration the line typically rises during exactly those cycles; Mesen2 reads it at the same point, calling `CheckForInterrupts()` after `ProcessHaltedState`'s `Idle()`. Before #3146 dispatch used the single end-of-cycle shadow, which dispatched one instruction early -- byuu's `test_irq.asm` sub-test 1 measures that 2-clock window and NESER captured `$EA` where hardware captures `$18`. See `interrupt_dispatch_tests::a_level_rising_inside_the_final_cycle_does_not_dispatch_at_that_boundary` (with its start-of-cycle companion, the two halves of byuu's bracket), `irq_level_that_wakes_wai_dispatches_in_the_same_step_call`, and `irq_level_during_wai_wakes_as_soon_as_the_preceding_cycle_observes_it`. Implied- and accumulator-mode opcodes route their single internal cycle through `tick_idle_or_dummy_read` (Mesen2 `IdleOrRead`): with an interrupt already imminent it becomes a dummy read at PC -- 8 master clocks in slow ROM against the idle's 6 -- selected by the `IMPLIED_OR_ACCUMULATOR_OPCODES` table transcribed from Mesen2's `RunOp` dispatch. The existing, separately-timed `irq_i_shadow` CLI/PLP/RTI-recognition-delay mechanic (#2985) is untouched and still composes via `irq_line_shadow && !irq_i_shadow`. `dispatch_hw_interrupt`'s two wasted cycles (dummy PC re-read + one internal tick, shared by NMI/IRQ/ABORT; BRK/COP absorb theirs into their own opcode+signature-byte fetch instead) and `op_pha`/`op_phx`/`op_phy`'s internal cycle both tick BEFORE their subsequent bus access (`tick_pre_access_internal_cycle`), matching Mesen2's `ProcessInterrupt`/`PHA`/`PHX`/`PHY` cycle ordering -- verified via a byte-level bus-trace diff against Mesen2 on KungFuFurby's `nmi.smc` (see `kungfufurby_nmi_tests.rs`/`kungfufurby_irq_tests.rs`). The rest of that family (PLA/PLX/PLY/PLB/PLD/PLP two idles before the pull, PHP/PHB/PHD/PHK one before the push) was completed in #3070, charged the same way so instruction totals are unchanged. The indexed addressing modes (`abs,X`, `abs,Y`, `(dp),Y`) now charge their penalty cycle inside the addressing mode, between forming the effective address and touching it, matching Mesen2's `AddrMode_AbsIdxX(isWrite)` -- the leftover-cycle model kept the total right but moved every indexed I/O access six clocks early (#3050). |
...
| `src/snes/ppu/background.rs` | BG tile pipeline for Modes 0-6: BGnHOFS/VOFS shared write-twice (BG_old) scroll latch, and the per-dot layer resolve (`resolve_pixel_pair` → main/sub `resolve_screen_pixel`, composed via `compose_pixels`) — 8x8/16x16 tile decode (2/4/8 bpp), 32x32/64x32/32x64/64x64 tilemaps with SC0..SC3 mirroring, H/V flip, per-tile + Mode-1 BG3 high priority, per-BG TM enable, Mode-0 per-BG palette regions, direct-color mode (CGWSEL bit 0) for 256-color BGs, offset-per-tile (modes 2/4/6 via BG3), and backdrop fallback. `resolve_screen_pixel` interleaves the OBJ priority levels (OBJ.0-3) with the BG layers via a `Slot` chart (fullsnes Background Priority Chart, gated by TM bit 4); `resolve_pixel_pair` dispatches to the Mode 7 resolver when `bg_mode == 7` (non-hires). True hires (modes 5/6, #3016): `bg_pixel_hires` fetches at doubled horizontal resolution (BGnHOFS in half-pixel units, every tilemap entry a forced 16-wide char N/N+1 pair, hflip mirroring across the pair, hires-domain OPT for mode 6 with the halved-value quirk), the sub screen samples the even and the main screen the odd half-pixel, mosaic collapses pairs to the even block-start sample, the sub screen displays TS layers regardless of CGWSEL bit 1 (with the DISPLAYED sub backdrop being CGRAM color 0), and `compose_hires_pair` applies per-dot hires color math (odd/main vs the pre-math sub color or COLDATA; even/sub vs the post-math main pixel at x-1 with the x-1 source gate). Pseudo-hires (SETINI bit 3) shares the interleave and math path. **Window masking** (#3011): `decode_window_registers` turns W12SEL/W34SEL/WOBJSEL, WH0-WH3 and WBGLOG/WOBJLOG into two [`WindowConfig`] values plus a per-layer mask-logic array on every write to those registers (the raw registers remain the save-state source of truth; the cache is rebuilt on restore). Within each `EIei` selector nibble the HIGH bit enables the window and the LOW bit inverts it, and the mask-logic bit pair is indexed by LAYER independently of which selector nibble that layer used. `window_area` combines the two windows with the per-layer operator only when BOTH are enabled -- exactly one enabled uses that window directly, neither masks nothing -- and `left > right` is an empty window. **Colour math** ordering follows Mesen2 `ApplyColorMathToPixel`: the CGWSEL bits 7-6 clip-to-black applies first and unconditionally (not gated by the CGADSUB layer bit), then the layer gate, then the bits 5-4 prevent-region (which names where maths is PREVENTED), then the add/subtract. Halving is decided by the caller, not read from CGADSUB bit 6, because hardware suppresses it both when the main pixel was clipped to black (uniformly, ares-style, clip mode 3 included) and for the empty-sub-screen fixed-colour fallback (#3012). In true hires the even/sub half takes that fallback from SUB coverage at **x-1** while its operand is the post-math MAIN pixel at x-1 (Mesen2 passes `prevX` to the same helper that indexes `_subScreenPriority`), so an empty sub screen one dot left substitutes COLDATA and drops halving there (#3035). Screen interlace (#3017): the modes 5/6 fetch doubles vertically too (realY = display line * 2 + field, incl. the OPT V arm), with the Mesen2 mosaic asymmetry (map row keeps the field term, chr row drops it) via split map/chr vertical offsets; mosaic also holds the OPT-replaced vertical offset in both the hires and native modes 2/4 paths. |
| `src/snes/ppu/sprites.rs` | OBJ (sprite) support (#2999, hardware pipeline per Mesen2/ares): OBSEL (`$2101`) size pairs (incl. undocumented 6/7), tile name base/gap; OAM priority rotation (OAMADDH bit 7); and the dot-incremental `ObjPipeline` — evaluation window (H=0..255 of the previous scanline, one OAM entry per 2 dots, priority-rotation order, 32-OBJ cap, horizontal-visibility test with the X=256 counts-but-never-draws quirk, forced blank pauses the cursor), reverse-order sliver fetch window (H=270..339, 34 8x1 CHR slots, off-screen columns skipped, the 35th attempted fetch raises STAT77 time over), dot-0 buffer swap compositing the presented `ObjLine` (CGRAM index resolved at query time), O(1) `obj_pixel_at` row-gated lookup (4bpp tile decode, X/Y flip, non-carrying large-tile composition, OBJ palettes at CGRAM 128+, 9-bit signed X), and dot-accurate STAT77 range over at `H = OAM_index x 2`. V-flip mirrors the line-within-sprite inside WIDTH-sized blocks (a plain `within_y ^= width - 1`), so the rectangular OBSEL 6/7 sizes flip as two stacked squares that mirror in place without swapping -- identical to a full-height mirror for the six square sizes, and the fix for wrong rows once Y wrapped (#3003; SNESdev wiki plus ares/ares-performance/higan/Snes9x, a deliberate Mesen2 divergence documented at the call site). The eval/fetch split stores only the OAM index, so each 2-dot fetch slot re-reads OAM and SETINI live and no range re-check happens -- matching Mesen2's `_spriteIndexes[32]` and ares-accurate's `{valid, index}`. A write in the H=256..269 gap therefore changes what that line samples, a write inside the fetch window changes only the remaining slivers, and a sprite made in-range during the gap still cannot join the frozen item list (#3026). |
...
| `src/snes/ppu/timing.rs` | Dot/scanline counters (1 dot per 4 master clocks, 341 dots/line, region-aware 262/312 frame lines; interlaced even fields latch one extra scanline at the frame wrap, #3017), scanline-specific short/long-line exceptions for NTSC/PAL, VBlank entry/exit (line 225 in 224-line mode, line 240 in 239-line mode), the NMI rising-edge model (`nmi_enable && nmi_flag`; an edge latched by the NMITIMEN write path carries a 2-cycle recognition arm, `update_nmi_line_arming`, #3081 — the sub-scanline clocks that drive it live in `ppu/irq.rs` since #3145), H/V counter latching (SLHV / WRIO falling edge) via `readable_h_position`, which derives the software-visible H position from `line_clock` the way Mesen2's `SnesPpu::GetCycle()` derives it from `hClock` -- a separate, deliberately lagged circuit from the render-dot counter across the paired long-dot compensation region (dots 323/324, 327/328; #3120), HBlank/VBlank flag timing, per-dot OBJ-pipeline advancement (`update_obj_pipeline`), and the DRAM-refresh (`dram_refresh_due`) and HDMA init/transfer (`hdma_init_due`/`hdma_transfer_due`, #2947) per-scanline trigger points, all phase-jittered by `total_master_clocks & 7`. |
| `src/snes/ppu/irq.rs` | The S-CPU interrupt counter circuit (#3144, #3145), ported from Mesen2's `InternalRegisters` `ProcessIrqCounters`/`UpdateIrqLevel`/`SetIrqFlag`. It ticks every 4 master clocks (intra-line clocks 2, 6, 10, ...) and drives **both** CPU interrupt signals, as the hardware does. H/V-IRQ (line-for-line): dedicated H/V counters, a continuous compare level against HTIME/VTIME, and a `need_irq` rising-edge countdown that sets TIMEUP one tick before raising the CPU-facing IRQ line — giving the semantics byuu's IRQ ROMs measure ($4211 reads within 4 clocks of the flag rising don't acknowledge, $4207-$420A writes re-evaluate the level at write time, enabling a mode whose compare already holds fires). NMI: the RDNMI flag rises at clock 2 of the first vblank scanline and falls at clock 2 of scanline 0, and when NMI is enabled the CPU NMI line follows at clock 6; the 4-clock gap is the $4210 read-hold window. The NMI half keeps two documented differences from Mesen2 (where the edge is latched when a `$4200` write lands mid-window, and deriving the NMI scanline live instead of latching it per frame) — see the `process_irq_counters` doc comment. Replaced the bsnes-style single-instant point compare, the `irq_edge_age` dispatch-delay approximation, and (in #3145) `timing.rs`'s separate `evaluate_nmi_flag_events` re-derivation of the same clocks. |
| `src/snes/ppu/framebuffer.rs` | Per-dot backdrop + composited BG/OBJ rendering into a BGR555 framebuffer with dynamic active height (224/239; the hi-res layout doubles to 448/478). `render_dot` resolves each dot's main/sub screen pair into per-scanline line buffers (`line_main`/`line_sub`/`line_main_final`) before finalizing into the framebuffer. **Hi-res layout vs content** (#3034): the *layout* comes from the frame-sticky `use_high_res_output` latch (Mesen2 `_useHighResOutput`) and the *content* from the live `hires_output_enabled()`, so a native dot inside a hi-res frame writes its composed pixel into both columns and a progressive hi-res frame duplicates each line into the next row. `convert_to_hires` (Mesen2 `ConvertToHiRes`, called from the `$2105`/`$2133` write paths) upgrades a frame mid-flight, re-laying-out every row already drawn (row y → rows 2y/2y+1, columns doubled, `line_inidisp` carried along); the upgrade is one-way and skips scanline 0 and VBlank, which the per-frame latch already covers. Also `screen_snapshot_rgb` (BGR555→RGB888 with INIDISP brightness/forced-blank applied per scanline via `line_inidisp`, latched at each row's first visible dot -- HDMA commonly rewrites INIDISP every scanline for fade/banding effects, #2947); it does no geometry normalization, since the rows were written in their final layout. |
...
| `src/snes/integration_tests/processor_tests_65816.rs` | Loader and harness for Tom Harte ProcessorTests 65816 JSON vectors. Parses upstream schema (`initial` / `final` / `cycles`), runs single-step CPU checks against a RAM-backed `SnesBus`, compares final CPU+RAM state and vector cycle count, and executes vectors from tracked subset files (`v1/*.json`) with per-filename overrides from local full-cache files (`full/v1/*.json`) when present. Opcodes listed in `CYCLE_EXACT_OPCODES` are additionally checked **cycle by cycle** against the bus activity the vector records — read/write/internal, address and value in order — decoding each vector cycle from its VDA/VPA/RWB signal string. This is the independent (non-Mesen2) oracle for where the CPU spends its internal cycles; the list is a migration front seeded with the direct-page and read-modify-write opcodes covered by #3068 and is meant to grow toward full coverage. 29 full-corpus vectors whose expectations are known-wrong for the 5A22 (emulation-mode direct-page pointer wrap, #3135) are skipped by exact name via `KNOWN_DIVERGENT_VECTORS`, whose doc comment carries the hardware evidence; running one directly still reports the divergence. |
...
| `src/snes/integration_tests/rom_runner.rs` | Shared headless SNES ROM runner used by future ROM-based suites. Loads generated or vendored `.sfc`/`.smc` bytes through `Snes`, runs with tick/frame budgets, supports pluggable `RunOracle` strategies — WRAM marker, bus-byte (for ROMs without marker support), and a screen-CRC golden oracle (`RunOracle::ScreenCrc` / `RunExitReason::ScreenCrcFrame`) that runs to a fixed frame and compares the rendered `screen_crc32()` against a human-approved value — returns diagnostics (exit reason, ticks, frames, PC, marker, screen CRC, and — on ScreenCrc runs that reached their target frame — the sampled frame's RGB888 pixels via `RunResult::screen_rgb`, for suites that must assert on pixels rather than a golden CRC, #3003), and optionally writes PNG captures under `target/snes_test_captures/` when `NESER_CAPTURE_SCREEN` is set. Interactive ROMs are driven via `RunConfig::with_input_script`: sorted frame-stamped `InputEvent`s applied once the completed-frame counter reaches each stamp (#2879), where each event carries an `InputAction` — a per-port pad button edge, SNES Mouse relative motion, a mouse button edge (#2886), or a Super Scope position/trigger/cursor/turbo/pause change (#2890). `RunConfig::with_controller_ports` selects the device type on each controller port (default standard pad) before the ROM loads, and `RunConfig::with_hardware` forces the video-timing region the way `snes-hardware` does for a frontend (default: auto-detect from the header country, #2888); `RunConfig::with_save_state_restore(frame, hardware)` carries a run across a save state onto a fresh console configured for another region once `frame` frames have completed (nr-273). `RunResult` also carries a `result_bytes` window over the 11 WRAM bytes behind the marker, so a fixture can return a measurement rather than only a pass/fail verdict, plus the run's active `screen_dimensions` and a direct `overscan_239_enabled` SETINI-bit readout (#3096) — the crop window's size tracks `screen_dimensions` (224 rows, or 448 in hi-res output) regardless of overscan, but overscan shifts the window's starting row by 7 (14 in hi-res), so `screen_dimensions`/`screen_crc32` alone cannot reliably tell a test whether overscan was actually engaged. |
| `src/snes/integration_tests/fixture_rom.rs` | Shared in-code LoROM fixture builder for the input (#2886/#2889), DMA/HDMA (#2884) and PAL/region (#2888) verification suites: emits 65816 programs as raw opcode bytes (immediate/absolute/long stores and loads, `and`/compares, long-branch-safe `bne_to`/`beq_to` poll loops, 8-bit index-register primitives with a `copy_block` indexed-loop helper, strobe pulses, MSB-first serial-bit reads, a read-only `place_data` region, force-blank toggles, `setup_gpdma`/`trigger_gpdma` and `setup_hdma`/`enable_hdma`/`disable_hdma` register programmers, a `country` header setter, an emulation-mode IRQ vector setter with `cli`/`wai`/`tsx` primitives for interrupt-handler fixtures (#3081), an `upload_spc_program` SPC700 IPL-boot-ROM transfer routine, and an inline `branch_fail_if_ne` fail-marker helper) into a 64 KiB LoROM image with the program origin at `$8200` so the canonical `rom_runner` marker idle loops at `$8100`/`$8110`/`$8120` stay clear. |
| `src/snes/integration_tests/neser_dma_tests.rs` | NESER-authored custom GPDMA fixtures (#2884), black-box against the fullsnes register spec: mode 0/1 to WRAM/VRAM, palette to CGRAM, words to OAM, A-bus increment/decrement/fixed, byte-count 0 == 65536, ascending multi-channel priority, and #2944 active-display write gating. VRAM/CGRAM/OAM readback instruments (composed from `fixture_rom` primitives) are each proven by a CPU write/read self-check. B->A direction (#3061): WMDATA `$2180` into cartridge SRAM, and the VRAM read port `$2139`/`$213A` into WRAM including its prefetch/increment side effects. |
| `src/snes/test_support.rs` | Test-only SNES construction helpers (`snes_test_config`/`snes_test_app_context`). Pins `ram_init_mode` to `Zero` in one place so every SNES test builds a deterministic console; `Config::default()` resolves to `Random` on desktop (#3128). |
| `src/snes/integration_tests/snes_ram_init_tests.rs` | Power-on RAM pattern coverage (#3128): drives `Snes::load_rom` with each `ram_init_mode` and asserts all six volatile RAMs (WRAM, VRAM, CGRAM, OAM, ARAM, SA-1 I-RAM) are filled per-mode and reproducibly per seed, that battery-backed save RAM is excluded, and that a hard reset re-applies the pattern while a soft reset preserves RAM. |
| `src/snes/integration_tests/cartridge_fixtures.rs` | In-code `CartFixture` builder for base-cartridge verification (#2885): emits minimal LoROM/HiROM/ExHiROM images (with an optional 512-byte copier header) carrying a spec-valid internal header and address-translation sentinels, authored from the SNES header spec rather than the loader. |
| `src/snes/integration_tests/neser_cartridge_tests.rs` | NESER-authored base-cartridge fixtures (#2885): loads the `cartridge_fixtures` images through the real `Cartridge`/bus paths to assert mapping detection, title/country metadata, SRAM size + battery, ROM address translation (sentinel read-back per mapping's banks/halves), and copier-header stripping; plus an executable LoROM battery-SRAM fixture (via `fixture_rom`) that writes two distinct SRAM addresses and self-checks the read-back. No-enhancement-chip cartridges only. |
| `src/snes/integration_tests/neser_pal_tests.rs` | NESER-authored PAL timing and video-region fixtures (#2888), authored from fullsnes and cross-checked against ares and Mesen2: STAT78 `$213F` bit 4 readback across every boundary in the fullsnes `$FFD9` country table plus both `snes-hardware` override directions; V-IRQ existence probes pinning the 262/312 scanline counts; an OPVCT latch on the VBlank rising edge showing VBlank starts at 225 (240 with SETINI overscan) in both regions; an SPC700 counter uploaded through the real IPL handshake measuring the 1.201808 PAL/NTSC frame-length x master-clock ratio; HDMA-banded screens compared across regions for identical dimensions and pixels (the two runs are each other's oracle, so no golden is committed); and auto-joypad reads on a PAL cartridge. nr-273 adds an NMI-driven game loop taking the same frames per logic step in both regions, an SA-1 loop counter measuring the 1.190846 frame-length-only ratio (the SA-1 runs off the SNES master clock, fullsnes "SNES Timing Oscillators"), and the SA-1 fixture carried across a save state onto the other region with an unchanged count. |
| `src/snes/integration_tests/neser_hdma_tests.rs` | NESER-authored custom HDMA fixtures (#2884): an HDMA channel targets WMDATA with WMADD preset so per-scanline transfers deposit into readable WRAM, verifying direct-mode line counter, non-repeat idle, repeat-every-line, indirect pointer deref, and terminator via the WRAM pass/fail marker. |
| `src/snes/integration_tests/kungfufurby_hdma_tests.rs` | KungFuFurby HDMA ROMs (#2884): `test_hdma`/`test_hdmasync`/`test_hdmatiming` all render the Mesen2-correct blue PASS screen (#3062, #3116, #3120). |
| `src/snes/integration_tests/jonasquinn_dma_tests.rs` | Canonical jonasquinn DMA/HDMA ROMs (#2884): `test_mdrhdma`, `hdma_midframe`, `test_dmavalid` (#3111), and `test_hdmadisable` (#3116) carry 0-pixel-diff Mesen2 goldens; `test_dmatiming_matches_mesen2` (0 px since #3127) and `test_dmatiming_latches_hv_after_gpdma` pass, as do the `test_hdmasync`/`test_hdmatiming` byuu mirrors (#3062, #3120). |
...
| `src/snes/integration_tests/input_super_scope_tests.rs` | Super Scope verification suite (#2890), spec-first against fullsnes/SNESdev and cross-checked against Mesen2: the 16-bit serial packet layout (fire, cursor, turbo, pause, offscreen/Null, signature ones), fire and pause single-shot vs cursor level, turbo toggle plus auto-fire, the offscreen/Null bit tracking on/off-screen aim, fire reported off-screen (shoot-to-reload), the issue's scripted example sequence, port-1/port-2 routing (`$4016`/`$4017`), and the aim-latch feature reading OPHCT/OPVCT (`$213C`/`$213D`) back with the Mesen2 centering offset (aimX+10, aimY-3). The aim latch itself is implemented as a beam-gated PPU location-latch request (mirroring Mesen2's `SetLocationLatchRequest`/`ProcessLocationLatchRequest`), armed once per frame from a Super Scope aimed on-screen with fire/cursor held. Fixtures are assembled in-code (no on-disk assets) and report through the WRAM pass/fail marker. |
| `src/snes/integration_tests/input_multitap_tests.rs` | Super Multitap verification suite (#2891), spec-first and cross-checked against Mesen2 (`Multitap::ReadRam`): the strobe-high detection signature (`$4017` reads `0x02` while OUT0 is high), a no-buttons baseline and per-slot isolation, the WRIO `$4201` bit-7 pair select with data1/data2 controller ordering, the issue's scripted example sequence (slot 1 A, slot 2 B, slot 3 Start, slot 4 Right, then all released) with held/released transitions, and the auto-joypad path latching the selected pair into JOY2/JOY4. The detection signature is implemented in `Multitap` (returns data1=0/data2=1 while the strobe is high); a `serial_read_pair` fixture helper reads both paired controllers' data lines in one pass. Fixtures are assembled in-code (no on-disk assets) and report through the WRAM pass/fail marker. |
...
| `src/snes/integration_tests/neser_obj_tests.rs` | NESER-authored OBJ feature suite (#2879): 14 bass-built static scenes (sources vendored beside the builds) covering all eight OBSEL size pairs, OBJ palettes, OBJ-vs-OBJ priority, OAM X bit 8, mode-1 OBJ-vs-BG layering, OAMADDH first-sprite rotation and OBJ Y wrap-around. All 14 goldens approved; 13 are Mesen2 cross-checks, while `obj-y-wrap.sfc`'s rests on ares/ares-performance/higan/Snes9x plus the SNESdev wiki because Mesen2 diverges on V-flip across a Y wrap (#3003), backed by a golden-independent structural assertion that derives the expected wrapped rows from the ROM's own output. |
| `src/snes/integration_tests/undisbeliever_ppu_window_tests.rs` | undisbeliever PPU window / INIDISP fade suite (#2880): input-scripted `window-mask-logic.sfc` (21 states) and `window-shapes-single.sfc` (14 shapes, A-tap lock against the 120-frame auto-advance), two free-running precalculated window demos, and 7 mid-plateau `inidisp_fadein_fadeout.sfc` samples. All 44 goldens are Mesen2-approved 0-px matches since #3011; the 14 shape vectors' scripted replay was first validated by matching each NESER capture against all 14 Mesen2 shape captures to prove the same shape index in both emulators. |
| `src/snes/integration_tests/neser_color_math_tests.rs` | NESER-authored colour-math/window/brightness suite (#2880): 11 bass-built ROMs over one Mode 1 quadrant scene (64 main x sub math crossings plus fallback regions) covering CGADSUB add/sub/half, COLDATA fixed-colour math, the transparent-sub fallback, the OBJ palette 4-7 rule, colour-window clip/prevent, layer window masks, and 17 INIDISP brightness samples. All 27 goldens are Mesen2-approved 0-px matches since #3011 (window decode, CGWSEL prevent regions) and #3012 (fallback halve suppression). |
...
| `src/snes/integration_tests/peterlemon_ppu_advanced_tests.rs` | PeterLemon advanced PPU mode suite (#2881): Mode 7 rotozoom (static + scripted rotate/zoom), HDMA Perspective and StarWars crawl, mosaic modes 3/5, six hires/interlace demos, four pseudo-hires HiColor demos. All 23 vectors carry Mesen2-approved goldens with none `#[ignore]`d (four pseudo-hires HiColor with #3016, re-approved at the native 512x448 with #3034; MosaicMode5 and the Interlace suite with #3017; Perspective and InterlaceSimpsonsHDMA with #3020; the StarWars crawl frames with #3021/#3050). |
...
| `src/snes/integration_tests/ddribin_hdrv_tests.rs` | ddribin HDRV display-mode suite (#2881, CC0, WLA-DX build): input-scripted test patterns; all four combos (colorbars, graybars, interlace, overscan) carry Mesen2-approved goldens (#3092) and are un-ignored. The overscan combo's screen-CRC is byte-identical to colorbars' (#3001's 224-row capture crop makes the extra overscan rows unobservable in a hash), so it additionally asserts `RunResult::overscan_239_enabled` directly to catch a broken SETINI path (#3096). |
| `src/snes/integration_tests/neser_opt_tests.rs` | NESER-authored offset-per-tile suite (#2881): 6 bass-built ROMs covering modes 2/4/6 OPT (flag gating, exempt first column, fine-scroll retention, mode 4 bit-15 H/V select). All 6 carry Mesen2-approved goldens (`opt-m6` since #3016, re-approved at the native 512x448 with #3034). |
| `src/snes/integration_tests/neser_hires_tests.rs` | NESER-authored mid-frame hi-res transition suite (#3034): 2 bass-built ROMs that HDMA a hi-res mode on at display line 100, one via BGMODE 1 → 5 and one via SETINI bit 3. No vendored ROM covers a mid-frame switch. Both match Mesen2 pixel-exactly at 512x448 and additionally assert the frame's structure (rows above the switch column-doubled, rows below carrying half-pixel pairs, every row pair identical), so a regression names what broke rather than only moving a CRC. |
| `src/snes/integration_tests/neser_irq_boundary_tests.rs` | NESER-authored end-to-end fixtures (#3081) pinning WHERE an already-pending IRQ dispatches around a `$420B` (MDMAEN) or `$4200` (NMITIMEN) write: a latched V-IRQ plus a `CLI ; STA $42xx ; WAI` sandwich makes the store the one instruction CLI's recognition shadow lets through, so the reference behaviour (dispatch preempts `WAI`; for `$420B` the started DMA runs inside the interrupt entry) is distinguishable from any instruction-granular deferral by one byte of stacked return PC, judged by the ROM's own IRQ handler through the marker protocol. Robust to boot alignment by construction; the identical images were Mesen2-verified (WRAM verdict + bus-trace diff) for #3081. |
...
| `src/snes/integration_tests/kungfufurby_nmi_tests.rs`, `kungfufurby_irq_tests.rs` | KungFuFurby's 2005-2008 NMI/H-V-IRQ test ROM collection (#2883/#3049, epic #2724). `demo_nmitest.smc`, `nmi.smc` (#3049's per-CPU-cycle NMI dispatch fix), `test_nmi.smc` (passing since #3116; its v1.1 test 27 is the hardware witness for the `SetNmiFlag(2)` enable-mid-vblank arm and caught #3081's first pure-deletion attempt), `test_irq4209.smc` (passing since #3116), and `demo_irqtest.smc` (#3049's per-CPU-cycle IRQ dispatch fix) all match Mesen2 exactly with approved goldens (pixel-verified at frame 600; `nmi.smc`'s bus-trace diff against Mesen2 is 99.99% byte-identical, 55957/55962 lines, across ~310k master clocks). `test_irq4200.smc` passes since #3144's IRQ counter circuit port; `test_irq.smc` (all ten sub-tests, verified through its `$700000`/`$700001` SRAM verdict) and `irq.smc` pass since #3146's cycle-start dispatch sample -- `irq.smc` had no source or disassembly and was slated for a Mesen2 trace diff, but shared that root cause and went green without any change aimed at it. `test_irqb.smc` passes since #3147, which found its two remaining sub-tests had unrelated causes: sub-test 4's OPHCT latch (`$0C` instead of `$10`) was the same cycle-start dispatch defect #3146 had already fixed, while sub-test 5's `jmp $217F` executes from the APU comm-port mirrors `$2144-$217F`, which `SnesSystemBus` decoded as open bus (see `read_mmio`/`write_mmio`/`dma_write_b_bus`). See `src/snes/cpu/cpu.rs`'s `resolve_nmi_arm_counter`/`poll_and_arm_nmi_edge`/`resample_irq_line`/`resample_irq_wai_line` for the dispatch mechanism. |
| `src/snes/integration_tests/sour_dma_irq_tests.rs` | Sour/SnesTests' `dma_irq_test.sfc` (#2883, epic #2724), rebuilt byte-identical from source (`ca65 -g` + `ld65 --dbgfile`) to recover its WRAM result-table address from debug symbols. Validates how many instructions run after a manual DMA (`$420B` write) before a pending IRQ/NMI dispatches, across 19 sub-cases; corrects a transcription error in the upstream README's expected-results table (`$00FF`, not `$FFFF`, for the two no-interrupt sentinels). `#[ignore]`d pending #3049: originally 8/19 sub-cases diverged (each off by exactly one fewer dispatched instruction than Mesen2, the same signature as the KungFuFurby suites); #3049's per-CPU-cycle NMI/IRQ dispatch fixes closed 2 of those 8 (both `CLI+INC` sub-cases), 6 remain. |
...
- Base screen width: 256 pixels (512 in hi-res/pseudo-hires/interlace modes)
- Active screen height: 224 or 239 lines (doubled to 448/478 whenever the
  frame uses the hi-res layout, i.e. hi-res/pseudo-hires or interlace)
- Output geometry follows one frame-sticky latch (#3034): a frame is 256x224
  or 512x448 as a whole, never a mix, even if the mode changes mid-frame
