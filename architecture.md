...
| `src/snes/ppu/timing.rs` | Dot/scanline counters (1 dot per 4 master clocks, 341 dots/line, region-aware 262/312 frame lines; interlaced even fields latch one extra scanline at the frame wrap, #3017), scanline-specific short/long-line exceptions for NTSC/PAL, VBlank entry/exit (line 225 in 224-line mode, line 240 in 239-line mode), the NMI rising-edge model (`nmi_enable && nmi_flag`; an edge latched by the NMITIMEN write path carries a 2-cycle recognition arm, `update_nmi_line_arming`, #3081), H/V counter latching (SLHV / WRIO falling edge), HBlank/VBlank flag timing, per-dot OBJ-pipeline advancement (`update_obj_pipeline`), and the DRAM-refresh (`dram_refresh_due`) and HDMA init/transfer (`hdma_init_due`/`hdma_transfer_due`, #2947) per-scanline trigger points, all phase-jittered by `total_master_clocks & 7`. |
| `src/snes/ppu/irq.rs` | H/V-IRQ counter circuit (#3144), a line-for-line port of Mesen2's `InternalRegisters` `ProcessIrqCounters`/`UpdateIrqLevel`/`SetIrqFlag`: dedicated H/V counters ticked every 4 master clocks (intra-line clocks 2, 6, 10, ...), a continuous compare level against HTIME/VTIME, and a `need_irq` rising-edge countdown that sets TIMEUP one tick before raising the CPU-facing IRQ line. Gives the semantics byuu's IRQ ROMs measure: $4211 reads within 4 clocks of the flag rising don't acknowledge, $4207-$420A writes re-evaluate the level at write time, and enabling a mode whose compare already holds fires. Replaced the bsnes-style single-instant point compare and the `irq_edge_age` dispatch-delay approximation. |
