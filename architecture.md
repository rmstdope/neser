...
| `src/snes/integration_tests/kungfufurby_nmi_tests.rs`, `kungfufurby_irq_tests.rs` | KungFuFurby's 2005-2008 NMI/H-V-IRQ test ROM collection (#2883, epic #2724). `demo_nmitest.smc` matches Mesen2 exactly with an approved golden. `nmi.smc`, `test_nmi.smc`, `irq.smc`, `test_irq.smc`, `test_irq4200.smc`, `test_irq4209.smc`, `test_irqb.smc`, `demo_irqtest.smc` are `#[ignore]`d with current CRCs recorded: NESER's interrupt dispatch resolves a few master clocks early relative to Mesen2 (#3049), an interrupt-pending-check granularity gap below current per-instruction dispatch resolution, partially improved by a one-instruction NMI dispatch delay fix landed with #2883 (`Cpu::step()` now snapshots `nmi_pending` before polling the bus edge, so a freshly-polled edge dispatches on the *next* `step()` call rather than the same one). The IRQ investigation confirmed IRQ shares NMI's root cause rather than being a separate bug. |
