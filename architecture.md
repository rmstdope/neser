...
| `src/snes/ppu/timing.rs` | Dot/scanline counters (1 dot per 4 master clocks, 341 dots/line, region-aware 262/312 frame lines), scanline-specific short/long-line exceptions for NTSC/PAL, VBlank entry/exit (line 225 in 224-line mode, line 240 in 239-line mode), the NMI rising-edge model (`nmi_enable && nmi_flag`), H/V counter latching (SLHV / WRIO falling edge), HBlank/VBlank flag timing, and per-dot OBJ-pipeline advancement (`update_obj_pipeline`). |
| `src/snes/ppu/framebuffer.rs` | Per-dot backdrop + composited BG/OBJ rendering into a BGR555 framebuffer with dynamic active height (224/239; interlace doubles to 448/478), and `screen_snapshot_rgb` (BGR555→RGB888 with INIDISP brightness/forced-blank at output). |
...
| `src/snes/apu/dsp/` | S-DSP pipeline split into focused submodules (`mod.rs`, `voice.rs`, `envelope.rs`, `brr.rs`, `gaussian.rs`, `echo.rs`) with per-voice state plus echo-ring/FIR state. Implements `$00-$7F` register behavior for ADSR/GAIN envelope stepping, KON/KOFF latency handling, NON noise routing with FLG-selected clocking, PMON adjacent-voice pitch modulation, ENVX/OUTX updates, BRR decode baseline, gaussian interpolation, pitch accumulation, per-voice/master volume helpers, and ARAM-backed echo semantics (ESA/EDL ring addressing, FIR tap mixing, EON/EFB/EVOL application, FLG mute/write-disable behavior, and delayed ESA application). |
...
| `src/snes/integration_tests/rom_pass_fail_spc700.rs` | SNES SPC700/APU ROM pass/fail suite. Defines `RomPassFailCase`/`RomPassFailOutcome`, dispatches cases through `rom_runner` with the requested oracle, and verifies the committed blargg ROM subset with the screen-CRC golden oracle (`verified_spc_apu_roms_pass_with_screen_crc_golden`, frame 600 goldens). Tracks the currently-failing ROMs via `candidate_roms()` typed by `BehaviorCategory` (Smp/Timers/Dsp), enforced to be documented in the manifest notes and the README-SNES candidate table. |
| `scripts/validate_snes_test_assets.py` | Strict SNES automated-test asset manifest validator. Enforces required provenance metadata (source/ref, license, oracle type, variant status/path, integrity, notes), committed-vs-optional corpus rules, and deterministic checksum conventions for committed CI subsets. |
| `scripts/refresh_snes_blargg_spc_apu_tests.py` | Optional-local SNES SPC/APU ROM corpus refresh helper referenced by the `snes-rom-pass-fail-blargg-spc-apu` manifest entry. Supports a `--dry-run` mode that prints the planned destination without network I/O. |
| `scripts/compute_snes_rom_asset_integrity.py` | Computes `tree_sha256` integrity metadata (sorted per-file `sha256  filename` records, file count, total size) for a committed SNES ROM asset directory; used to (re)generate the committed `rom_pass_fail` manifest integrity. |
