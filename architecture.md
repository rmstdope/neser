...
| `src/snes/apu/mod.rs` | `SnesApu` bootstrap path: owns 64 KB ARAM, embedded/override IPL ROM selection, SPC700 CPU stepping via master-clock catch-up, `$2140-$2143` <-> `$F4-$F7` port handshake latches, and save-state capture/restore of APU bootstrap state. |
