...
#### 3.1 Public Fields in Core Structs
...
**Why This Is a Smell:**
- Allows external code to mutate internal state directly
- No validation when setting values
- Difficult to refactor internals without breaking API

**Proposed Refactoring:**
- Make fields private and provide getter/setter methods
- Add a `CpuState` struct for inspection:
...
The comment acknowledges this is exposed for MemController access.

**Proposed Refactoring:**
- Add specific methods to the PPU for I/O bus access:

```rust
impl Ppu {
    pub fn io_bus(&self) -> u8 { self.registers.io_bus() }
    pub fn set_io_bus(&mut self, value: u8) { self.registers.set_io_bus(value); }
}
```
...
#### 4.1 Mapper Trait is Well-Designed ✓

**Location:** `src/cartridge/mapper.rs`

The `Mapper` trait is well-designed with appropriate default implementations:
...
This is a good example of trait usage in the codebase.

#### 4.2 Missing Memory Access Trait

**Proposed Enhancement:**
Consider a `BusDevice` trait for memory-mapped I/O:
...
- Cleaner memory controller implementation
- Easier testing with mock devices

---

## 5. Enums Instead of Flags

### Current Issues Found

#### 5.1 TV System Enum is Well-Designed ✓
...
This is a good example of using enums instead of booleans.

#### 5.2 Interrupt Kind Enum is Well-Designed ✓
...
**Proposed Refactoring:**
Consider using a `bitflags!` macro for APU channel enable flags:
