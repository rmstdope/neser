...
(Dungeon Master) and the DSP-4 (Top Gear 3000) are emulated.
...
- `dsp_tests.rs` -- the DSP-1 (nr-auv), DSP-2 (nr-608) and DSP-4 (nr-tfq). Nintendo's
  ...
  both LoROM windows, a DSP-2 ("DUNGEON MASTER") cartridge in its
  `20-3F`/`A0-BF` window, and a DSP-4 ("TOP GEAR 3000") cartridge in its
  `30-3F`/`B0-BF` window. The run treats the synthetic image as genuine.
  Real-game checks with a player's `dsp1b.rom`/`dsp2.rom`/`dsp4.rom` stay manual.
