...
//! * Backdrop fill from the first palette entry for forced-blank or
//!   unimplemented modes.
//!
...
/// Result of stepping the PPU — flags telling the bus which DMA hooks
/// (V-Blank / H-Blank) to fire after a step.
///
...
/// caller (the bus) routes them.
#[derive(Debug, Default, Clone, Copy)]
pub struct PpuStepEvents {
    /// A V-Blank period started during this step (entered scanline 160).
    pub vblank_started: bool,
    /// An H-Blank period started on a *visible* scanline during this
    /// step. H-Blank also occurs during V-Blank scanlines but only
    /// visible-scanline H-Blanks trigger H-Blank-mode DMA per GBATek.
    pub hblank_started: bool,
    /// A complete frame finished during this step — the framebuffer is
    /// ready for the frontend to read. Cleared by the consumer.
...
impl Ppu {
    ...
    /// timing, DISPSTAT flags, and IRQs via `ic`. Returns edges the bus
    /// must propagate (V-Blank / H-Blank DMA hooks, frame complete).
    ///
    ...
    fn advance_scanline(&mut self, ic: &mut InterruptController, events: &mut PpuStepEvents) {
        ...
        if next == lyc {
            ...
            if self.dispstat & dispstat::VCOUNT_IRQ_ENABLE != 0 {
...
mod tests {
    ...
    fn dispstat_status_bits_are_read_only() {
        ...
        // must be ignored — the PPU owns those bits.
        ppu.write_dispstat(0x0007 | 0x0038 | 0xAB00);
        assert_eq!(ppu.read_dispstat() & dispstat::STATUS_MASK, 0);
    ...
    fn hblank_irq_fires_only_when_enabled() {
        ...
        ppu.write_dispstat(dispstat::HBLANK_IRQ_ENABLE);
        ppu.step(CYCLES_PER_SCANLINE, &mut ic, &make_vram(), &make_pram());
    ...
    fn vblank_irq_fires_when_enabled() {
        let mut ppu = Ppu::new();
        let mut ic = make_ic();
        ppu.write_dispstat(dispstat::VBLANK_IRQ_ENABLE);
        let cycles = CYCLES_PER_SCANLINE * VISIBLE_SCANLINES;
    ...
    fn vcount_match_flag_and_irq() {
        ...
        ppu.write_dispstat(dispstat::VCOUNT_IRQ_ENABLE | (5 << 8));
        // Step to scanline 5.
    ...
    #[test]
    fn mode3_with_bg2_disabled_outputs_black() {
        let mut ppu = Ppu::new();
        let mut ic = make_ic();
        let mut vram = make_vram();
        let pram = make_pram();
        ...
        ppu.write_dispcnt(3);
        // Paint VRAM red — should NOT appear because BG2 is off.
        vram[0] = 0x1F;
        ppu.step(
            CYCLES_PER_SCANLINE * SCANLINES_PER_FRAME,
            &mut ic,
        ...
        assert_eq!(&ppu.framebuffer()[0..3], &[0, 0, 0]);
    }
