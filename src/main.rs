...
fn render(cpu: &cpu::Cpu, pixels: &mut Pixels) {
    ...
    for y in 0..TEXTURE_HEIGHT {
        for x in 0..TEXTURE_WIDTH {
            ...
            let memory_value = cpu.memory.read(memory_index as u16);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    event_loop.run(move |event, _, control_flow| match event {
        ...
        Event::WindowEvent {
            ...
            VirtualKeyCode::Up => {
                cpu.memory.write(0xff as u16, 0x77);
            }
            VirtualKeyCode::Down => {
                cpu.memory.write(0xff as u16, 0x73);
            }
            VirtualKeyCode::Left => {
                cpu.memory.write(0xff as u16, 0x61);
            }
            VirtualKeyCode::Right => {
                cpu.memory.write(0xff as u16, 0x64);
            }
            _ => {}
        },
        Event::WindowEvent {
        ...
        Event::MainEventsCleared => {
            // println!("Executing opcode at PC: {:04X}, OpCode: {:02X}", cpu.pc, cpu.memory.read(cpu.pc as usize as u16));
            // println!("A: {:02X}, X: {:02X}, Y: {:02X}, SP: {:02X}, P: {:08b}", cpu.a, cpu.x, cpu.y, cpu.sp, cpu.p);
            for _ in 0..256 {
                cpu.run_opcode();
                cpu.memory.write(0xfe as u16, rand::random::<u8>());
            }
