...
#[test]
#[ignore = "debug-only: dumps PCM12 sub-test results for align test investigation"]
fn debug_dump_channel_1_align() {
    // CorrectResults from channel_1_align.asm — 6 rows × 8 bytes = 48.
    #[rustfmt::skip]
    let expected: &[u8] = &[
    ...
    dump_samesuite_sweep_failures(&format!("{BASE}/channel_1/channel_1_align.gb"), expected);
}
...
fn debug_dump_channel_4_align() {
    ...
    dump_samesuite_sweep_failures(&format!("{BASE}/channel_4/channel_4_align.gb"), expected);
}
