//! Mapper 317 – Unknown board
//!
//! Specifications:
//! - NesDev wiki: unavailable due to network restriction (Cloudflare 403).
//! - Fallback: Mesen2 `MapperFactory.cpp` – mappers 316–318 are listed as
//!   `//316-318` with no `case` entries (not implemented; board name unknown).
...
/// Mapper 317 – Unknown board (minimal stub)
///
...
mod tests {
    ...
    fn mapper_317_is_registered() {
        ...
        assert!(
            result.is_ok(),
            "Mapper 317 must be registered in the factory"
        );
    }
    ...
    fn power_on_prg_ffff_reads_bank_0_last_byte() {
        ...
        let mapper = Mapper317::new(MapperContext::new_for_test(
            317,
            prg,
            chr,
            NametableLayout::Horizontal,
    ...
    fn power_on_chr_1fff_reads_bank_0_last_byte() {
        ...
        let mut mapper = Mapper317::new(MapperContext::new_for_test(
            317,
            prg,
            chr,
            NametableLayout::Horizontal,
    ...
    fn irq_never_pending() {
        let mut mapper = make_mapper();
        mapper.write_prg(0x8000, 0xFF);
        assert!(!mapper.irq_pending(), "Mapper 317 must never assert IRQ");
    }
    ...
    fn registers_snapshot_is_empty() {
        ...
        assert!(
            ...
            "Mapper 317 has no registers; snapshot must be empty"
        );
    }
