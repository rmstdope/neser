...
mod tests {
    ...
    /// With a Gamepad returning 0 on all bits and open_bus = $BF,
    /// the result should be $A0 (bits 5,7 from open bus), not $BE.
    #[test]
    fn test_gamepad_open_bus_only_on_bits_5_to_7() {
        let reads = Rc::new(RefCell::new(0));
        let dummy_reads = Rc::new(RefCell::new(0));
        let controller: Rc<RefCell<Box<dyn Controller>>> = Rc::new(RefCell::new(Box::new(
            TestController::new(reads.clone(), dummy_reads.clone()),
        )));
        ...
        // With open_bus = $BF and controller returning 0,
        // bits 5-7 should come from open_bus ($BF -> 101xxxxx = $A0)
        // bits 0-4 should come from controller (all 0)
        let result = device.read(0x4016, 0xBF, false).unwrap();
        assert_eq!(
            result, 0xA0,
            "Expected $A0 (only bits 5-7 from open bus), got ${:02X}",
