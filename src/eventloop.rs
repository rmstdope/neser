...
impl EventLoop {
    ...
    /// let headless = EventLoop::new(true, TvSystem::Ntsc, 1.0, 1.0, true, None, false, false, None, None)?;
    ///
    /// // Create an EventLoop with an NTSC window at 2x scale
    /// let ntsc = EventLoop::new(false, TvSystem::Ntsc, 2.0, 1.0, true, None, false, false, None, None)?;
    ///
    /// // Create an EventLoop with a PAL window at 3x scale at 2x speed with gamepads
    /// let pal = EventLoop::new(false, TvSystem::Pal, 3.0, 2.0, true, None, true, false, None, None)?;
    /// # Ok::<(), String>(())
    /// ```
    pub fn new(
        headless: bool,
...
mod tests {
    ...
    fn test_breakpoint_hit_pauses_and_opens_debugger() {
        let mut event_loop =
            EventLoop::new(true, TvSystem::Ntsc, 1.0, 1.0, true, None, false, false, None, None).unwrap();
    ...
    fn test_eventloop_creation() {
        let event_loop =
            EventLoop::new(true, TvSystem::Ntsc, 1.0, 1.0, true, None, false, false, None, None);
    ...
    fn test_new_headless() {
        let event_loop =
            EventLoop::new(true, TvSystem::Ntsc, 2.0, 1.0, true, None, false, false, None, None);
        assert!(event_loop.is_ok());
    }
    ...
    fn test_scaling_below_minimum() {
        let event_loop =
            EventLoop::new(true, TvSystem::Ntsc, 0.5, 1.0, true, None, false, false, None, None);
        assert!(event_loop.is_ok());
    }
    ...
    fn test_scaling_above_maximum() {
        let event_loop =
            EventLoop::new(true, TvSystem::Ntsc, 6.0, 1.0, true, None, false, false, None, None);
        assert!(event_loop.is_ok());
    }
    ...
    fn test_scaling_at_maximum() {
        let event_loop =
            EventLoop::new(true, TvSystem::Ntsc, 5.0, 1.0, true, None, false, false, None, None);
        assert!(event_loop.is_ok());
    }
    ...
    fn test_gamepad_enabled_no_controllers_present() {
        ...
        let event_loop =
            EventLoop::new(true, TvSystem::Ntsc, 1.0, 1.0, true, None, true, false, None, None);
